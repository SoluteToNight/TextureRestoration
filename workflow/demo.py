...
def arg_parser() -> Namespace:
    ...
    parser.add_argument("--tile",action="store_true",help="tile")
...
def main() -> None:
    ...
    workflow.CCSR(img_list, scale=2).process()
    for img in img_list:
        if img.tmp_data is None:
            raise ValueError(f"Image {img.name} has no tmp_data after Diffusion process")
        if not isinstance(img.tmp_data, Image.Image):
            raise ValueError(f"Image {img.name} has an invalid tmp_data after Diffusion process")
